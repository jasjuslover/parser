  ...
  (function () {
    var favicon = "favicon",
      ...
      root =
        ...
        slash;
    for (
      var i = 0, nodes = document.querySelectorAll(".ui-option"), node;
      (node = nodes[i]);
      i++
    ) {
    ...
    (function () {
      ...
      document.querySelector(".popup-centerer").addEventListener(
        event_click,
        function (e) {
      ...
      document
        .querySelector("#bitcoin")
        .addEventListener(event_click, function () {
      ...
      $result.addEventListener(
        ...
        function (ev) {
          ...
          if (
            ...
            clickedElement.classList.contains("toggle-end")
          ) {
    ...
    function parse(str) {
      ...
      function parseArray(snatch) {
        ...
        function parseElement(snatch) {
          ...
          return snatch
            .err("Comma is missing", snatch.shift(snatch.search(/(,|\])/)))
            .update("</li>");
        }
        if (snatch.indexOf("[") === -1) {
        ...
        if (snatch.indexOf("]") === -1) {
          ...
          snatch.update(
            '</ol><span class="toggle-end" card="' + (io + 1) + '"></span>'
          );
          return snatch.update("</span>");
        }
        snatch.shift(1);
          ...
          for (k = 0; k < length; k++) {
            if (value.substring(k, k + 1) == "\\") {
              if (k + 1 < length) {
                k++;
                if (
                  !value.substring(k, k + 1).search(/[^\"|\\|\/|b|f|n|r|t|u]/)
                ) {
      ...
      var hasError = false,
        Snatch = function (todo) {
          ...
          this.swap = function (charNumber) {
            if (
              charNumber &&
              !isNaN(Number(charNumber)) &&
              this.todo.length >= charNumber
            ) {
              this.update(
                this.todo.substr(0, charNumber),
                this.todo.substring(charNumber)
              );
            }
            return this;
          };
          ...
          this.err = function (title, text) {
            hasError = true;
            return this.update(
              '<span class="error" title="' + title + '">' + text + "</span>"
            );
          };
          this.shift = function (nbOfChars) {
            ...
            if (
              nbOfChars &&
              !isNaN(Number(nbOfChars)) &&
              this.todo.length >= nbOfChars
            ) {
      ...
      if (ltrim(str).substr(0, 1) === "[") {
        ...
        if (ltrim(str).substr(0, 1) === "{") {
          ...
          result = {
            html: snatch
              .err("JSON expression must be an object or an array", str)
              .update(null, "")
              .toString(),
            valid: false,
          };
